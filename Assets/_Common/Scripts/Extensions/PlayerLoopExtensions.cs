...
namespace ShadersMagic
{
    public static class PlayerLoopExtensions
    {
        ...
        public static void ModifyCurrentPlayerLoop(ModifyPlayerLoop playerLoop)
        {
            var currentLoop = PlayerLoop.GetCurrentPlayerLoop();
            playerLoop(ref currentLoop);
            PlayerLoop.SetPlayerLoop(currentLoop);
        }

        public static ref PlayerLoopSystem GetSystem<TSystem>(ref this PlayerLoopSystem playerLoop)
        {
            var type = typeof(TSystem);
            for (int i = 0; i < playerLoop.subSystemList.Length; i++)
            {
                ref var playerLoopSystem = ref playerLoop.subSystemList[i];
